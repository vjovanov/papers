...
In function \code{dot}, if the vector sizes are constant the inner product can
...
{\bf Staging at host language compile time.} How can we use staging for programs whose values are statically
 ...
 since host language run-time values comprise the majority of user programs~(\sct{sct:discussion}).
