...
In the section we informally present \tool, a staging extension for Scala based on the compile-time views.
 \tool is a compiler plugin that executes in a phase after the
 Scala type checker. The plugin takes as input pre-typed Scala programs and uses
...
annotations \code{ct} and \code{inline}, and functions \code{ct} and \code{inline}.
...
the translation to the compile-time views (\figref{fig:ct-translation}), changes
...
\end{table*}
 ...
 according to the translation to the compile-time view (\figref{fig:ct-translation})
