...
To satisfy cross-stage persistence (\sct{sct:wf-ctv}) the \code{pow} must be \code{@inline}.
However, to make reduce the number of required annotations we implicitly add the \code{inline} annotation
when at least one parameter or the result type is marked as \code{ct}. In the example
...
inlined due to the \code{inline} annotation, then the function zero gets
inlined. Since \code{dnum} returns the compile-time view of \code{DoubleNumerc}
the method \code{zero} is evaluated at compile time. The constant \code{0.0} is
promoted to \code{ct} since \code{DoubleNumeric} is a compile time view
(formally defined in \sct{sct:compile-views}). Finally the \code{ct(0.0)} result
...
partially evaluated through staging. We start with the desugared \code{dot} function
