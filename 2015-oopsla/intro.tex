...
Programming languages Idris and D allow placing the \code{static} annotation on
 ...
 information about \code{static} terms can not be propagated through return types of functions,
 so \code{static} in Idris and D is more a partial evaluation construct.
...
a priory no operations exist \code{Rep} annotated types. For example, in the \code{dot}
...
This, in effect, requires that only vector arguments (not their elements) are
 ...
 that can be further used for partial evaluation. Residual programs of \code{dot}
...
We have implemented a prototype system according to the translation
 ...
 open-sourced\footnote[3]. It has a minimal Scala
 interface (\sct{sct:interface}) based on type annotations. We have evaluated the
...
\footnotetext[3]{Source and releases are at \url{https://github.com/scala-inline/}.}

% Include the reasoning behind staging.
