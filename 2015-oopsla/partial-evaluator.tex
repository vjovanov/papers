...
\smartparagraph{Annotation inline} can be used only at the term level on statically
 ...
 compilation time. \code{inline} is marking that the function application is a compile-time
 ...
 Internally \code{inline} can be expressed in terms of \code{ct}. A method:\begin{lstparagraph}
@inline def dot[V: Numeric]
  (v1: Vector[V], v2: Vector[V]): V
\end{lstparagraph}
  will have a type method type \code{((v1: Vector[V], v2: Vector[V]) => V)@ct}. This
  type, however, can not be written by users. We choose the name \code{inline} to
  be consistent with the existing Scala \code{inline} annotation.



% TODO classes (only if we must)
...
\smartparagraph{Functions \code{ct} and \code{inline}} are used at the term level
 ...
 the \code{ct} we would not be able to instantiate compile-time views of types.
