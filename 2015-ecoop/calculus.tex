...
% TODO discuss with Sandro the problems with monotonicity.
\begin{figure}[H]
  \infax[\textsc{W-Any}]
  {\wff{\i{Any}}}
...
% Questions/TODOs:
%  - Can we push wff down to the place where we do explicit inline. This would minimize the formulaz if it works out.
%  - Introduce a lemma that show that after reduction types remain well-formed!
...
%  - Introduce what is a literal at all!
\begin{figure}[H]
  ...
  {\Gamma \ts \inline{t}: \inline{T}}

  \infrule[\textsc{T-Dynamic}]
  {t $ is not literal$ \ \ \ \ \Gamma \ts t: \i{T}}
  ...
  {\Gamma \ts t_1: \i{([X <: \j{T_{11}}] \ra \k{T_{12}})}  \ \ \ \ \Gamma \ts \l{T_2} <: \j{T_{11}}   \ \ \ \  \Gamma \ts \i{} <: \l{}}
...
\begin{figure}[H]
  ...
  {t_1 \pe t_1' \ \ \ \ t_1' $ is not inline$ \ \ \ \ t_2 \pe t_2'}
  {t_1(t_2) \pe t_1'(t_2')}
